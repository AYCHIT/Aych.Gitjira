...
module.exports = function mapToJira (payload, authorMap) {
  const commits = payload.commits.map(commit => mapCommit(commit, authorMap[commit.author.username])).filter(commit => commit)
